...
public class MaxMinBallAction_instr {
			...
			for(int i = 0; i != one.size()+two.size()+three.size(); i++) {
				...
				if(i < one.size()) {
					...
					exec(two.get(i-one.size()));
						minCheck = new MinMaxBallAction_instr(stadium, team.getEnemyTeam());
						minCheck.progress(checkingDepth-1);
						twoHigh[i-one.size()] = minCheck.getWorstAvancement();
					...
					exec(three.get(i-one.size()-two.size()));
						minCheck = new MinMaxBallAction_instr(stadium, team.getEnemyTeam());
						minCheck.progress(checkingDepth-1);
						threeHigh[i-one.size()-two.size()] = minCheck.getWorstAvancement();
	...
	public static void main(String args[]){
		...
		System.out.println("Taille de One: "+test.getOne().size());
		System.out.println("Taille de Two: "+test.getTwo().size());
		System.out.println("Taille de Three: "+test.getThree().size());
