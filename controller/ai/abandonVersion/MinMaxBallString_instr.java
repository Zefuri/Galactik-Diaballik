...
public class MinMaxBallString_instr{
	Stadium stadium;
	Team team;
	CoupString coup;

	public MinMaxBallString_instr(Stadium stadium, Team team, int alpha, int beta) {
	...
	public void progress(int checkingDepth){
		...
		if(checkingDepth != 0){
			MaxMinBallString_instr maxCheck;
			...
			for(int i = 0; coup.canAccess() && i != coup.numberOfAction(); i++){
				...
				coup.exec(i);
					maxCheck = new MaxMinBallString_instr(stadium, team.getEnemyTeam(), coup.getAlpha(), coup.getBeta());
